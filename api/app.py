...
def calculate_buccal_fat(landmarks):
    ...
    try:
        ...
        score = np.interp(combined_metric, [0.02, 0.05, 0.10, 0.15], [10.0, 9.5, 7.0, 4.0])
        final_score = score * 0.9 + 3.0
        return round(np.clip(final_score, 5.0, 10.0), 1)
    except Exception as e:
        print(f"Buccal Fat error: {str(e)}")
        return 7.5

def calculate_under_eye(image, landmarks):
    """Under-eye analysis with better normalization"""
    try:
        def get_under_eye_mask(eye_points):
            points = [(int(p.x * image.shape[1]), int(p.y * image.shape[0])) 
        ...
        def analyze_eye(mask):
            ...
            face_light = np.percentile(l_channel, 70)
            eye_light = np.percentile(region, 30)
        ...
        darkness_score = 10 * (1 - sigmoid(avg_dark * 4 - 1))
        yellow_score = 10 * (1 - sigmoid(avg_yellow * 3 - 1.5))
        final_score = (darkness_score * 0.5) + (yellow_score * 0.3) + 4.5
        return round(np.clip(final_score, 5.5, 10.0), 1)
    except Exception as e:
